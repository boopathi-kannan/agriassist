...
const Cart = () => {
  const{products,currency,cartItems,removeFromCart,getCardCount,updateCartItem,navigate,getCartAmount,axios,user,setCartItems}=useAppContext();
  ...
  const placeOrder=async()=>{
    console.log("place trigged")

    try {
        if(!selectedAddress){
            return toast.error("place Select the address")
        }
              ...
              {cartArray.map((product, index) => (
                  <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                      ...
                      className="cursor-pointer mx-auto">
                         <img src="https://res.cloudinary.com/dve8r06ul/image/upload/v1746966636/wrong_iln84k.webp" alt="remove" className="inline-block w-6 h-6"/>
                      </button>
                  </div>)
              )}

              <button onClick={()=>{navigate('/products');scrollTo(0,0)}} className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                 <img src="https://res.cloudinary.com/dve8r06ul/image/upload/v1746983508/left-arrow-2294064-1939032_hfdlwn.webp" alt="arrow" className="group-hover:translate-x-1 transition h-6 w-6"/>
                  Continue Shopping
              </button>
          ...
          <div className="max-w-[360px] w-full bg-gray-100/40 p-5 max-md:mt-16 border border-gray-300/70">
              ...
              <div className="mb-6">
                  ...
                  <div className="relative flex justify-between items-start mt-2">
                      ...
                      <button onClick={() => setShowAddress(!showAddress)} className="primary hover:underline cursor-pointer">
                          Change
                      </button>
                      {showAddress && (
